...
`packet-svc-router-gate` is centered on backend services work in Kotlin. The technical target is: Design a Kotlin verification harness for router systems, covering protocol validation, framed sample traffic, and failure-oriented tests.
...
The score is deterministic. The current threshold is 152. Risk has a penalty of 4, latency has a penalty of 2, and weight contributes a bonus of 6.
